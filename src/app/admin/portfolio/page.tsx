...
export default function AdminPortfolioPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await adminJson<{ portfolio: AdminPortfolio[] }>('/api/admin/portfolio', token);
    ...
    <div className="space-y-6">
      ...
      {!loading && items.length > 0 && (
        <ul className="divide-y divide-white/10 border border-white/10 rounded-xl overflow-hidden">
          {items.map((p) => (
            <li key={p.id}>
              <Link href={`/admin/portfolio/${p.id}`} className="flex items-center justify-between gap-4 px-4 py-4 hover:bg-white/5">
                <div>
                  <p className="font-medium">{p.title}</p>
                  <p className="text-xs text-zinc-500">{p.category || 'Sin categoría'} · Orden {p.order}</p>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full ${p.isPublished ? 'bg-primary/20 text-primary' : 'bg-zinc-800 text-zinc-400'}`}>
